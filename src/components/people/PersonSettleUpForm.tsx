...
export default function PersonSettleUpForm({ contactId, onDone }: { contactId: string; onDone?: () => void }) {
  ...
  useEffect(() => {
    fetch(`/api/people/${contactId}/debts`)
      .then((r) => r.json())
      .then((data) => {
        const list: PersonDebt[] = data.debts || [];
        setDebts(list);
  ...
  const showGroupNames = new Set(debts.map((d) => d.groupId)).size > 1;
  ...
  async function record(i: number) {
    ...
    setSavingIndex(i);
    const result = await apiPost(`/api/groups/${debt.groupId}/settlements`, {
        ...
        <div key={i} className="flex items-center gap-2 rounded-md border border-border p-3">
          <div className="flex-1 text-sm text-text">
            ...
            <div className="text-xs text-text-faint">
              Up to {d.amount.toFixed(2)} {d.currency}
              {showGroupNames && ` · ${d.groupName}`}
            </div>
          </div>
          <input
            type="number"
