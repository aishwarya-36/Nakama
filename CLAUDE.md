...
- Money always flows through `GroupMember`, never directly against `User` — this lets a guest be "claimed" by a real account later without touching history. `src/lib/contacts.ts`'s `resolveMember` is that claim path: adding a person by an email that matches an existing `User` links `GroupMember.userId` straight to their real account (no `Contact` created) instead of the usual guest-`Contact` flow, so the group/expense shows up in their own account immediately. Only triggers on an explicit email match at add-time — an existing `Contact` is never retroactively re-linked just because its stored `email` happens to match someone who signs up later
